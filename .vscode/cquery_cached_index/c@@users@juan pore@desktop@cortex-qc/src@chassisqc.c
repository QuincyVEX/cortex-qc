...
#include "chassisQC.h" // redundant, but ensures that the corresponding header file (chassis.h) is included
#include "portDef.h" // All port defintions on the cortex
#include "lcdQC.h"

void chassisSet(int left, int right) {
  motorSet(wheel_right, left);
  motorSet(wheel_left, -right);
  ...
  int kp = 7;
   ...
   //Repeat ten times a second.
    while(true)
    {
      ...
      motorSet(wheel_left, masterPower);
      motorSet(wheel_right, -slavePower);
...
void driveForDistancePID(int distance, int speed) {
  ...
  motorSet(wheel_left, 0);
  motorSet(wheel_right, 0);
